...
/**************************************************************/
/*  Simulation of Marienbad's game                            */
/**************************************************************/

sauve :- qsave_program('marienb',[goal=game,autoload=true]).
...
Players take as many items they want in a single row
      ...
      * * * * *
    * * * * * * *
The one who takes the last item loses.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
...
gameOver([1|L], 1) :- gameOver(L, 0).	% one single item left: user loses if other rows are empty
...
askMove(State, NState) :-
        ...
        move(State, NState).    
askMove(State, NState) :-
	repeat0, 	% repeat0 acts as a bouncing wall for backtracking
	write('Your turn: '), nl, 
	write('Number of X? '),
...
% and valuates each of them depending on whether the winner or the loser went through it
keepInMind([], _).
keepInMind([H|HR], 0) :-
	memory(H, 1), 
	!, 
