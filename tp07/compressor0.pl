...
/* ------------------------------------------------------* /
    This program implements a simple compressor that
...
incrementor(_, 0, L, L).
incrementor(X, Times, [Y|Ys], Rest) :-
    plusOne(X,Y),
    incrementor(Y, Times1, Ys, Rest),
    Times is Times1 + 1.
...
compress(Input,[cs,Times,X,Step|CRest]) :-
    ...
    Times > 1,
    compress(Rest,CRest).
compress([X|Xs], [inc, X, Times|CRest]) :-
    incrementor(X, Times, Xs, Rest),
    Times > 1,
    compress(Rest, CRest).
